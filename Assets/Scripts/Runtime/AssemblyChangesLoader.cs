...
using Memory = UnityReloader.Runtime.Polyfills.Memory;
...
namespace UnityReloader.Runtime
{
    ...
    public class AssemblyChangesLoader: IAssemblyChangesLoader
    {
                    ...
                    if (ProjectTypeCache.AllTypesInNonDynamicGeneratedAssemblies.TryGetValue(createdTypeNameWithoutPatchedPostfix, out var matchingTypeInExistingAssemblies))
                    {
                        ...
                        if (!editorOptions.IsDidFieldsOrPropertyCountChangedCheckDisabled
                            ...
                            && (DidFieldsOrPropertyCountChanged(createdType, matchingTypeInExistingAssemblies)
                                || DidSerializedFieldSignatureChange(createdType, matchingTypeInExistingAssemblies)))
                        {
        ...
        private static bool DidFieldsOrPropertyCountChanged(Type createdType, Type matchingTypeInExistingAssemblies)
        {
            ...
            if (createdTypeFieldAndProperties.Count != matchingTypeFieldAndProperties.Count)
            {
                ...
                LoggerScoped.LogError($"It seems you've added/removed field to changed script. This is not supported and will result in undefined behaviour. Hot-reload will not be performed for type: {matchingTypeInExistingAssemblies.Name}" +
                               $"\r\n\r\nYou can skip the check and force reload anyway if needed, to do so go to: 'Tools -> Unity Reloader -> Start Screen -> Reload -> tick 'Disable added/removed fields check'" +
                ...
                LoggerScoped.Log(
                    ...
                    $"\r\n - Open Settings 'Tools -> Unity Reloader -> Start Screen -> New Fields -> tick 'Enable experimental added field support'");
                return true;
            }
        ...
        // Detects changes that affect Unity serialization / the Inspector even when the field count is unchanged
        // (e.g. toggling [SerializeField], changing a field's type, or public/private visibility). These cannot be
        // applied by method detour and require a full recompile.
        private static bool DidSerializedFieldSignatureChange(Type createdType, Type matchingTypeInExistingAssemblies)
        {
            return !GetSerializedFieldSignatures(createdType)
                .SetEquals(GetSerializedFieldSignatures(matchingTypeInExistingAssemblies));
        }

        private static HashSet<string> GetSerializedFieldSignatures(Type type)
        {
            var result = new HashSet<string>();
            foreach (var f in type.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
            {
                // Mirrors Unity's serialization rule closely enough for change detection:
                // public or [SerializeField] instance fields, excluding [NonSerialized].
                var isSerialized = (f.IsPublic || f.IsDefined(typeof(SerializeField), true))
                                   && !f.IsDefined(typeof(NonSerializedAttribute), true);
                if (isSerialized)
                    result.Add(f.Name + ":" + (f.FieldType.FullName ?? f.FieldType.Name));
            }
            return result;
        }
