...
namespace UnityReloader.Editor.NewFields
{
    [InitializeOnLoad]
    public class NewFieldsRendererDefaultEditorPatch
    {
        ...
        {
            if ((bool)UnityReloaderPreference.EnableExperimentalAddedFieldsSupport.GetEditorPersistedValueOrDefault())
            {
            ...
            if (__instance.target)
            {
                if (TemporaryNewFieldValues.TryGetDynamicallyAddedFieldValues(__instance.target, out var addedFieldValues))
                {
                    ...
                    try
                    {
                        ...
                        var redirectedType = AssemblyChangesLoader.Instance.GetRedirectedType(__instance.target.GetType());
                        if (redirectedType == null) //type not yet cached for redirect (e.g. before first hot-reload of this type) - skip rendering instead of throwing
                            return;

                        var newFieldNameToGetTypeFn = CreateNewFieldInitMethodRewriter.ResolveNewFieldsToTypeFn(redirectedType);
