...
using UnityReloader.Editor.AssemblyPostProcess;
...
namespace UnityReloader.Editor.Compilation
{
    ...
    public class DotnetExeDynamicCompilation : DynamicCompilationBase
    {
        ...
        public static CompileResult Compile(List<string> filePathsWithSourceCode, UnityMainThreadDispatcher unityMainThreadDispatcher)
        {
            ...
            {
                // UnityReloaderManager has a special case for reporting SourceCodeHasErrorsException.
...
2) Click Unity Reloader -> Add / Open User Script Rewrite Override
...
*If you want to prevent that message from reappearing please go to Tools -> Unity Reloader -> Start Screen -> Logging -> tick off 'Log how to fix message on compilation error'*");
